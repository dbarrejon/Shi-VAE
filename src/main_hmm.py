...
# Shi-VAE
args.model_name = "{}_{}_{}z_{}h_{}s_{}miss".format(
    args.model, args.dataset, args.z_dim, args.h_dim, args.K, args.percent_miss
)
...
X, Z, theta, mask, index = dset.load_data_pickle(
    args.dataset, args.data_dir, percent_miss=args.percent_miss
)
index_train, index_val, index_test = index

data_train = dset.HMMDatset(
    X, mask, types_list=types_list, Z=Z, theta=theta, index=index_train
)
data_valid = dset.HMMDatset(
    X, mask, types_list=types_list, Z=Z, theta=theta, index=index_val
)
data_test = dset.HMMDatset(
    X, mask, types_list=types_list, Z=Z, theta=theta, index=index_test
)
...
if args.train == 0 or args.train == -1:
    ...
    test_loader = torch.utils.data.DataLoader(
        data_test, batch_size=64, shuffle=False, collate_fn=dset.standard_collate
    )
